...
def place_selected_items(user_id, selected_items, payment_method, coupon_code=None):
    ...
    try:
        ...
        # Call the procedure for each selected item
        for _ in selected_items:
            cursor.callproc('PlaceOrderFromCart', [user_id, delivery_partner_id])

        conn.commit()

        # Update payment method and coupon code after order creation
...
def show_login_signup():
    ...
    choice = st.sidebar.selectbox("Select", ["Login","Signup","Admin Login"])
...
def show_admin_portal():
    ...
    with tabs[0]:
        ...
        if not rest_df.empty:
            rest_name = st.selectbox("Select Restaurant to Delete", rest_df['name'])
            if st.button("Delete Selected Restaurant"):
    ...
    with tabs[1]:
        rest_df = get_restaurants()
        if not rest_df.empty:
            rest_name = st.selectbox("Select Restaurant", rest_df['name'])
            rest_id = int(rest_df[rest_df['name'] == rest_name]['restaurant_id'].values[0])
            menu_df = get_menu_by_restaurant(rest_id)
            st.dataframe(menu_df)

            st.markdown(" Add, Delete & Update Menu Item")
            ...
            if st.button("Add Item"):
                conn = get_connection()
                cur = conn.cursor()
                cur.execute("INSERT INTO Menu (restaurant_id, name, category, price, stock) VALUES (%s,%s,%s,%s,%s)",
            ...
            if not menu_df.empty:
                menu_name = st.selectbox("Select Item to Update or Delete", menu_df['name'])
                menu_id = int(menu_df[menu_df['name'] == menu_name]['menu_id'].values[0])
                new_price = st.number_input("New Price", 0)
                new_stock = st.number_input("New Stock", 0)
            ...
            for order_id, group in grouped:
                ...
                with col1:
                    if status not in ["Delivered", "Cancelled"] and st.button(
                        f"Mark Delivered #{order_id}", key=f"adm_del_{order_id}_{random.randint(1,9999)}"):
                        update_order_status(order_id, "Delivered")
                        st.success(f"Order #{order_id} marked Delivered")
                        rerun_app()
                with col2:
                    if status not in ["Delivered", "Cancelled"] and st.button(
                        f"Cancel #{order_id}", key=f"adm_can_{order_id}_{random.randint(1,9999)}"):
...
def show_restaurants_dropdown_menu():
    ...
    for _, row in restaurants.iterrows():
        with st.expander(f"{row['name']} - {row['address']}"):
            ...
            # Show restaurant image if available with fixed width and height
            img_path = restaurant_images.get(row['name'])
            if img_path:
                img = Image.open(img_path)
                img = img.resize((500, 300))  # fixed width=500px, height=300px
            ...
            if not reviews_df.empty:
                ...
                for _, rev in reviews_df.iterrows():
                    st.write(f"{rev['user_name']} ({rev['review_date'].strftime('%Y-%m-%d')}): ⭐{rev['rating']}\n> {rev['comment']}")
...
def show_cart():
    ...
    if cart_df.empty:
        ...
        return
    selected_items = st.multiselect("Select items to order", cart_df['item_name'])
    ...
    payment = st.selectbox("Payment Method", ["Credit Card","Debit Card","UPI","Wallet","Cash"])
    coupon = st.text_input("Coupon (optional)")
    if st.button("Place Selected Order"):
...
# --------------------------
# ORDER HISTORY with dropdown for rating/review
# --------------------------
...
def main():
    ...
    if 'admin' in st.session_state and st.session_state['admin']:
        ...
        st.sidebar.button("Logout", key="user_logout", on_click=lambda: (st.session_state.clear(), rerun_app()))
        choice = st.sidebar.radio("Navigation", ["Browse Restaurants","Cart","Orders"])
